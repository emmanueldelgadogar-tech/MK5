...
export default function Home() {
  ...
  // ✅ Barra de progreso (Opción 3)
  const DURATION = 5000; // 5s por promo (igual que antes)
  const [promoIndex, setPromoIndex] = useState(0);
  const [promoProgress, setPromoProgress] = useState(0);

  // ✅ PROMOS REALES (iniciales) + fit para que NO recorte (contain)
              ...
              >
                <img
                  ...
                  className={`promo-main ${
                    promoActiva?.fit === "contain" ? "is-contain" : "is-cover"
                  }`}
                  alt={promoActiva?.alt || "Promoción"}
                />
              </button>

              {/* ✅ dots opcionales (se quedan, son chiquitos y útiles) */}
